...
// --- ??/???? ---
// ????????(????)
#define MAX_PHYSICAL_SPEED_RADS 99999.0f 
...
void multi_encoder(void)
{
    ...
    const int pri_gear = 32; // 主编码器齿轮数
    const int sec_gear = 31; // 从编码器齿轮数
    if(init == 0)
    {
        init = 1;
...
//		Velocity_Filtered = Encoder.vel/2608.917197/12.0f;
	  Velocity_Filtered = get_angular_velocity_rads_v3(Encoder.raw, 50)/GEAR_RATIO;
