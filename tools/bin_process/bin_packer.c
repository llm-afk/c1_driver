...
static void append_version_suffix(const char *path, uint16_t version, char *out_path, size_t out_size)
{
    const char *last_dot;
    const char *last_slash;
    const char *last_backslash;
    ...
    snprintf(suffix, sizeof(suffix), "_%u", version);
    ...
    if (last_dot != NULL && (separator == NULL || last_dot > separator)) {
        base_len = (size_t)(last_dot - path);

        if (base_len + strlen(suffix) + strlen(last_dot) + 1 > out_size) {
            safe_copy_string(out_path, out_size, path);
            return;
        }

        memcpy(out_path, path, base_len);
        out_path[base_len] = '\0';
        strcat(out_path, suffix);
        strcat(out_path, last_dot);
    } else {
        if (strlen(path) + strlen(suffix) + 1 > out_size) {
            safe_copy_string(out_path, out_size, path);
            return;
        }
        safe_copy_string(out_path, out_size, path);
...
static int run_pack_mode(const Options *options)
{
    ...
    char final_output_path[PATH_BUFFER_LEN];
    ...
    append_version_suffix(options->output_path, software_version, final_output_path, sizeof(final_output_path));
