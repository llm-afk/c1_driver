...
clear
echo -e "${CYAN}======================================================================${RESET}"
echo -e "${BOLD}${WHITE}             GD32C103CB J-Link SWD Auto Flashing Tool            ${RESET}"
echo -e "${CYAN}======================================================================${RESET}"
echo

log "GD32C103CB自动烧录脚本启动" "INFO"
...
# 使用数组包裹通配符，让 Bash 自动检索并展开所有匹配的文件
BOOT_FILES=(firmware/dgm_boot*.bin)
APP_FILES=(firmware/dgm_app*.bin)

# 取匹配到的第一个文件（即使名字里带版本号后缀也能准确抓到）
...
if [ -f "$LOCAL_BOOT" ]; then
    # 使用 basename 只打印出纯文件名，让日志更干净好看
    log "找到 Bootloader 固件: $(basename "$LOCAL_BOOT")" "INFO"
else
    log "Bootloader 固件 (dgm_boot*.bin) 丢失！" "ERR"
    MISSING_FIRMWARE=1
fi
...
if [ -f "$LOCAL_APP" ]; then
    ...
    log "Application 固件 (dgm_app*.bin) 丢失！" "ERR"
    MISSING_FIRMWARE=1
fi

if [ $MISSING_FIRMWARE -eq 1 ]; then
    echo
    echo "请将 'dgm_boot*.bin' 和 'dgm_app*.bin' 固件直接放入 'tools/auto_flash/linux/firmware/' 文件夹。"
    echo
    read -p "按回车退出..."
    exit 1
fi
...
get_device_state() {
    ...
    done < <("$JLINK_PATH" -device GD32C103CB -if SWD -speed 4000 -autoconnect 1 -ExitOnError 1 -NoGui 1 -CommanderScript detect.jlink 2>&1)
...
EOF
        ...
        done < <("$JLINK_PATH" -device GD32C103CB -if SWD -speed 4000 -autoconnect 1 -ExitOnError 1 -NoGui 1 -CommanderScript flash.jlink 2>&1)
