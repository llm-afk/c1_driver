...
if [ -n "$LOCAL_BOOT" ]; then
    ...
    log "未在 firmware 目录下找到以 dgm_boot_released_C2_GD*.bin 命名的固件！" "ERR"
    MISSING_FIRMWARE=1
fi
...
if [ -n "$LOCAL_APP" ]; then
    ...
    log "未在 firmware 目录下找到以 dgm_app_released_C2_GD*.bin 命名的固件！" "ERR"
    MISSING_FIRMWARE=1
fi

if [ $MISSING_FIRMWARE -eq 1 ]; then
    echo
    echo "请确保 tools/auto_flash/linux/C2_GD/firmware/ 目录下存在正确命名的 dgm_boot_released_C2_GD*.bin 和 dgm_app_released_C2_GD*.bin 打包固件。"
    echo
    read -p "按回车退出..."
    exit 1
fi
