...
clear
echo -e "${CYAN}======================================================================${RESET}"
echo -e "${BOLD}${WHITE}           C2_PRO J-Link SWD Auto Flashing Tool          ${RESET}"
echo -e "${CYAN}======================================================================${RESET}"
echo
...
WIN_FIRMWARE_DIR="$SCRIPT_DIR/../../windows/C2_PRO/firmware"
BOOT_DIR="$SCRIPT_DIR/../../../../Firmware_boot/MDK-ARM/object"
APP_DIR="$SCRIPT_DIR/../../../../Firmware_app/MDK-ARM/object"

# Check Bootloader
BOOT_FILES_LOCAL=("$LOCAL_FIRMWARE_DIR"/dgm_boot_released_C2_PRO*.bin)
BOOT_FILES_WIN=("$WIN_FIRMWARE_DIR"/dgm_boot_released_C2_PRO*.bin)
BOOT_FILES_OBJ=("$BOOT_DIR"/dgm_boot_released_C2_PRO*.bin)
...
if [ -n "$LOCAL_BOOT" ]; then
    ...
    log "未在 tools/auto_flash 目录或编译输出目录找到以 dgm_boot_released_C2_PRO*.bin 命名的固件！" "ERR"
    MISSING_FIRMWARE=1
fi

# Check App
APP_FILES_LOCAL=("$LOCAL_FIRMWARE_DIR"/dgm_app_released_C2_PRO*.bin)
APP_FILES_WIN=("$WIN_FIRMWARE_DIR"/dgm_app_released_C2_PRO*.bin)
APP_FILES_OBJ=("$APP_DIR"/dgm_app_released_C2_PRO*.bin)
...
if [ -n "$LOCAL_APP" ]; then
    ...
    log "未在 tools/auto_flash 目录或编译输出目录找到以 dgm_app_released_C2_PRO*.bin 命名的固件！" "ERR"
    MISSING_FIRMWARE=1
fi

if [ $MISSING_FIRMWARE -eq 1 ]; then
    echo
    echo "请确保分别编译了 Bootloader 和 Application 项目，并且对应的 object/ 目录下生成了对应的 dgm_boot_released_C2_PRO*.bin 和 dgm_app_released_C2_PRO*.bin 打包固件。"
    echo
    read -p "按回车退出..."
    exit 1
fi
