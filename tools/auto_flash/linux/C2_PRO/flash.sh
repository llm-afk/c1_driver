...
# Check Bootloader
BOOT_FILES=("$BOOT_DIR"/dgm_boot_released_C2_PRO*.bin)
if [ -f "${BOOT_FILES[0]}" ]; then
    LOCAL_BOOT="${BOOT_FILES[0]}"
    log "找到 Bootloader 固件: $(basename "$LOCAL_BOOT")" "INFO"
else
    log "未在 Firmware_boot/MDK-ARM/object 目录下找到以 dgm_boot_released_C2_PRO*.bin 命名的固件！" "ERR"
    MISSING_FIRMWARE=1
fi

# Check App
APP_FILES=("$APP_DIR"/dgm_app_released_C2_PRO*.bin)
if [ -f "${APP_FILES[0]}" ]; then
    LOCAL_APP="${APP_FILES[0]}"
    log "找到 Application 固件: $(basename "$LOCAL_APP")" "INFO"
else
    log "未在 Firmware_app/MDK-ARM/object 目录下找到以 dgm_app_released_C2_PRO*.bin 命名的固件！" "ERR"
    MISSING_FIRMWARE=1
fi
