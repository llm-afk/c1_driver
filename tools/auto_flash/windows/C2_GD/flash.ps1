...
if (Test-Path $firmwareDir) {
    $bootFiles = Get-ChildItem -Path $firmwareDir -Filter "dgm_boot_released_C2_GD*.bin" -File
...
# Check App
if (Test-Path $firmwareDir) {
    $appFiles = Get-ChildItem -Path $firmwareDir -Filter "dgm_app_released_C2_GD*.bin" -File
