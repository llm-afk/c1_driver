...
Write-Host "             C2_GD J-Link SWD Auto Flashing Tool            " -ForegroundColor White -Bold
Write-Host "======================================================================" -ForegroundColor Cyan
Write-Host ""
...
if ($bootDir -and (Test-Path $bootDir)) {
    ...
    if ($bootFiles.Count -ge 1) {
        ...
        Log-Message "未在 Firmware_boot/MDK-ARM/object 目录下找到以 dgm_boot_released_C2_GD*.bin 命名的固件！" "ERR"
...
if ($appDir -and (Test-Path $appDir)) {
    ...
    if ($appFiles.Count -ge 1) {
        ...
        Log-Message "未在 Firmware_app/MDK-ARM/object 目录下找到以 dgm_app_released_C2_GD*.bin 命名的固件！" "ERR"
        $missingFirmware = $true
    }
} else {
