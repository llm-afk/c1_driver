...
if (-not $localBoot) {
    Log-Message "未在 firmware 目录下找到以 dgm_boot_released_A2*.bin 命名的固件！" "ERR"
...
if (-not $localApp) {
    Log-Message "未在 firmware 目录下找到以 dgm_app_released_A2*.bin 命名的固件！" "ERR"
    $missingFirmware = $true
}

if ($missingFirmware) {
    Write-Host ""
    Write-Host "请确保 tools\auto_flash\windows\A2\firmware\ 目录下存在正确命名的 dgm_boot_released_A2*.bin 和 dgm_app_released_A2*.bin 打包固件。"
    Write-Host ""
    Write-Host "按任意键退出..."
    [void]$Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
    exit 1
