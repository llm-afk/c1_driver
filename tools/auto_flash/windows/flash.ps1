...
Write-Host "             Unified J-Link SWD Auto Flashing Tool            " -ForegroundColor White -Bold
Write-Host "======================================================================" -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path $firmwareDir) {
    $bootFiles = Get-ChildItem -Path $firmwareDir -Filter "dgm_boot_released_*.bin" -File
...
if (-not $localBoot) {
    Log-Message "未在 firmware 目录下找到以 dgm_boot_released_*.bin 命名的固件！" "ERR"
    $missingFirmware = $true
}

# Check App
if (Test-Path $firmwareDir) {
    $appFiles = Get-ChildItem -Path $firmwareDir -Filter "dgm_app_released_*.bin" -File
...
if (-not $localApp) {
    Log-Message "未在 firmware 目录下找到以 dgm_app_released_*.bin 命名的固件！" "ERR"
    $missingFirmware = $true
}

if ($missingFirmware) {
    Write-Host ""
    Write-Host "请确保 tools\auto_flash\windows\Unified\firmware\ 目录下存在正确命名的 dgm_boot_released_*.bin 和 dgm_app_released_*.bin 打包固件。"
    Write-Host ""
    Write-Host "按任意键退出..."
    [void]$Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
    exit 1
